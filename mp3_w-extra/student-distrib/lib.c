...
void enable_cursor()
{
	outb(CURSOR_ONE, CURSOR_CMD);	
	outb((inb(CURSOR_DATA) & CURSOR_THREE) | CURSOR_START, CURSOR_DATA);
 
	outb(CURSOR_TWO, CURSOR_CMD);	
	outb((inb(CURSOR_DATA) & CURSOR_FOUR) | CURSOR_END, CURSOR_DATA);
}
/*  
 * update_cursor
 * update the cursor's location (in terminal)
 * inputs: x position
 *         y curposition 
 * outputs: none
 * return value: none
...
void update_cursor(int x, int y){
	...
	outb(CURSOR_LOWER, CURSOR_STATUS_PORT);
	outb((uint8_t) (pos & MASK_LOWER), CURSOR_DATA_PORT);
	outb(CURSOR_UPPER, CURSOR_STATUS_PORT);
    /* 8: shift by 8 bits to get the upper byte */
	outb((uint8_t) ((pos >> SHIFT_8) & MASK_LOWER), CURSOR_DATA_PORT);
}

/**void clear(void);
 * Input: void
 * Output: Clears video memory
 * Return Value: none
 * Side Effect: Reset (screen_x,screen_y) and (cursor_x,cursor_y) to (0,0)
 */
void clear(void) {
    int32_t i;
...
void fresh_video(void)
{
    int32_t i;
    for (i = 0; i < NUM_ROWS * NUM_COLS; i++) {
        uint8_t temp = *(uint8_t *)(video_mem + (i << 1));
        *(uint8_t *)(video_mem + (i << 1)) = temp;
        *(uint8_t *)(video_mem + (i << 1) + 1) = ATTRIB;
    } return;
}
...
void backspace(void) {
    ...
    int offset = current_terminal->cursor_y * NUM_COLS + current_terminal->cursor_x;
    if (offset <=0) return;
    offset--;
    ...
    *(uint8_t *)((char*)current_terminal->screen_buffer + (offset << 1) + 1) = ATTRIB;
    ...
    offset++;
    current_terminal->cursor_y = offset / NUM_COLS;
    current_terminal->cursor_x = offset % NUM_COLS;
...
void putc(uint8_t c) {
    ...
    screen_x = current_run_terminal->cursor_x;
    screen_y = current_run_terminal->cursor_y;
    video_mem_local = (char*) current_run_terminal->screen_buffer;
    
    /* if current running process's terminal is currently displaying terminal */
    if (get_active_pcb()->terminalid == current_active_termid) display_flag = 1;
    ...
    // implement scrow down
    if (NUM_ROWS == screen_y){
        if(display_flag) memmove(video_mem, video_mem + NUM_COLS * 2, NUM_COLS * (NUM_ROWS - 1) * 2);
        else             memmove((char*)video_mem_local, (char*)video_mem_local + NUM_COLS * 2, NUM_COLS * (NUM_ROWS - 1) * 2);
        // clear the bottom row
        if(!display_flag){
            for (i = NUM_COLS * (NUM_ROWS - 1); i < NUM_ROWS * NUM_COLS; i++){
    ...
    current_run_terminal->cursor_x = screen_x;
    current_run_terminal->cursor_y = screen_y;
    if (display_flag)
        update_cursor(current_run_terminal->cursor_x, current_run_terminal->cursor_y);
}
...
/**
 * 
 * This function is implemented to serve for keyboard input on screen
 * void putk(uint8_t c);
 * Inputs: uint_8* c = character to print
 * Return Value: void
 * Function: Output a character to the console
...
/* void turn_on_rtc(void)
 * Inputs: void
 * Return Value: void
 * Function: make rtc_swich on */
void turn_on_rtc(void)
{
    rtc_switch = 1;
    return;
